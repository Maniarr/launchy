...
impl Output {
    ...
    pub fn set_mode(&mut self, mode: LaunchpadMode) -> Result<(), crate::MidiError> {
        let mode = match mode {
            LaunchpadMode::Live => 0,
            LaunchpadMode::Programmer => 1,
        };
    
        return self.send(&[240, 0, 32, 41, 2, 13, 14, mode, 247]);
    }

    pub fn set_brightness(&mut self, level: u8) -> Result<(), crate::MidiError> {
        return self.send(&[240, 0, 32, 41, 2, 13, 8, level, 247]);
    }
